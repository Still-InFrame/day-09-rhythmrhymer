...
# Exclude THIS script: its PATTERNS line contains literals (e.g. SERVICE_ROLE)
# that match themselves and hard-block every publish. betterleaks still scans it.
HITS="$(git grep --cached -nIE "$PATTERNS" -- ':(exclude)publish.sh' 2>/dev/null || true)"
if [ -n "$HITS" ]; then
